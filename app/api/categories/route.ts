    ...
    try {
        ...
        if (!type || (type !== "inbound" && type !== "outbound")) {
            return NextResponse.json(
                { message: "Parâmetro 'type' é obrigatório e deve ser 'inbound' ou 'outbound'" },
                { status: 400 }
            );
        }

        const categories = await getCategoriesByType(type as "inbound" | "outbound");
...
export async function POST(request: Request) {
    try {
        ...
        if (!name || !type) {
            return NextResponse.json({ message: "Campos 'name' e 'type' são obrigatórios" }, { status: 400 });
        }

        if (type !== "inbound" && type !== "outbound") {
            return NextResponse.json({ message: "Campo 'type' deve ser 'inbound' ou 'outbound'" }, { status: 400 });
        }
