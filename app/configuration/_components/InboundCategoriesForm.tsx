...
export default function InboundCategoriesForm() {
  ...
  async function fetchCategories() {
    try {
      ...
      const response = await fetch("/api/categories?type=inbound");
  ...
  async function handleSubmitForm(e: FormEvent) {
    ...
    try {
      const response = await fetch("/api/categories", {
        ...
        body: JSON.stringify({ ...form, type: "inbound" }),
      });
  ...
  return (
    <main className="px-4 mt-4">
      <GroupDown title="Categorias de Entrada">
